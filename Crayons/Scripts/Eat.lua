...
local CrayonRedRenderables = { "$CONTENT_DATA/Tools/Renderables/char_crayon_red.rend" }

local RenderablesEattoolTp = { "$SURVIVAL_DATA/Character/Char_Male/Animations/char_male_tp_eattool.rend", "$CONTENT_DATA/Tools/Renderables/char_eattool_tp.rend" }
local RenderablesEattoolFp = { "$SURVIVAL_DATA/Character/Char_Male/Animations/char_male_fp_eattool.rend", "$CONTENT_DATA/Tools/Renderables/char_eattool_fp.rend" }

sm.tool.preloadRenderables( CrayonRedRenderables )
sm.tool.preloadRenderables( RenderablesEattoolTp )
sm.tool.preloadRenderables( RenderablesEattoolFp )

local FoodUuidToRenderable =
{
	["37525f80-f807-495d-a814-d87803a168b1"] = { rend = CrayonRedRenderables, colour = sm.color.new("#ff0000") }, --Crayon Red
}
...
function Eat.cl_loadAnimations( self )
	...
	self.tpAnimations = createTpAnimations(
			self.tool,
			{
				...
				putdown = { "Putdown" },
				paint = { "Paint", { nextAnimation = "idle" } }
			}
		)
		...
		if self.tool:isLocal() then
			self.fpAnimations = createFpAnimations(
				self.tool,
				{
					...
					paint = { "Paint", { nextAnimation = "idle",  blendNext = 0.2 } }
				}
			)
		end
...
function Eat.cl_updateEatRenderables( self )
	...
	local itemData = FoodUuidToRenderable[tostring( self.activeItem )]
	if itemData then
		eatRenderables = itemData.rend
	else
	...
	local colour = itemData and itemData.colour
	if itemData then
...
-- Interact
function Eat:sv_paint(args)
	---@type Shape, Uuid
	local shape, item = args.shape, args.item
	local colour = FoodUuidToRenderable[tostring(item)].colour

	if not sm.exists(shape) or shape.color == colour then return end

	if sm.item.isBlock(shape.uuid) and shape:getBoundingBox() ~= sm.vec3.one() * 0.25 then
		local point = shape:getClosestBlockLocalPosition(args.point)
		shape:destroyBlock(point, sm.vec3.one())

		local coloured = shape.body:createBlock(shape.uuid, sm.vec3.one(), point)
	...
	sm.container.beginTransaction()
	sm.container.collect(self.tool:getOwner():getInventory(), item, 1)
	sm.container.endTransaction()

	self.network:sendToClients("cl_paint", args.point)
end

function Eat:cl_paint(pos)
	sm.effect.playEffect("PaintTool - Paint", pos)
	sm.particle.createParticle("paint_smoke", pos, sm.quat.identity(), FoodUuidToRenderable[tostring(self.activeItem)].colour)

	setTpAnimation(self.tpAnimations, "paint", 1)
	if self.tool:isLocal() then
		setFpAnimation(self.fpAnimations, "paint", 0.2)
	end
end

function Eat.client_onEquippedUpdate( self, primaryState, secondaryState, forceBuildActive )
	if not forceBuildActive and not self.eating then
		local hit, result = sm.localPlayer.getRaycast(7.5)
		if result.type == "body" then
			local shape = result:getShape()
			if sm.exists(shape) and shape.color ~= FoodUuidToRenderable[tostring(self.activeItem)].colour then
				sm.gui.setInteractionText( "", sm.gui.getKeyBinding( "Create", true ), "Paint" )
				if primaryState == 1 or primaryState == 2 then
	...
	elseif primaryState == sm.tool.interactState.stop and self.eating then
		self:stopEat()
		self.network:sendToServer( "sv_n_stopEat" )
	end
