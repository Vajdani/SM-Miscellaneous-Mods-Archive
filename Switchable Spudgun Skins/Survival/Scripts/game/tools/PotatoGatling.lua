...
local skins = {
	...
	barrel = {
		renderables[2],
		"$CONTENT_a1e5756a-83a1-4c2f-8667-21874cb9b490/Skins/Barrel/Barrel_spinner/blue/char_spudgun_barrel_spinner.rend",
		"$CONTENT_a1e5756a-83a1-4c2f-8667-21874cb9b490/Skins/Barrel/Barrel_spinner/green/char_spudgun_barrel_spinner.rend"
	}
}

--cant have them in one table because of renderable preloading
local skinNames = {
	base = {
		"normal"
	},
...
for v, rendGroup in pairs(skins) do
	sm.tool.preloadRenderables( rendGroup )
end

function PotatoGatling:server_onCreate()
	...
	if self.data == nil then
		self.data = {
			...
			sightIndex = 1,
			barrelIndex = 1,
			--could be the first index of every group in *skins* as well, this is just shorter lol
...
function PotatoGatling.client_onCreate( self )
	...
	self.gui = sm.gui.createGuiFromLayout( "$CONTENT_a1e5756a-83a1-4c2f-8667-21874cb9b490/SkinSwitch.layout" )
	self.gui:setButtonCallback( "skinBaseDec", "cl_base" )
	self.gui:setButtonCallback( "skinBaseInc", "cl_base" )

	self.gui:setButtonCallback( "skinStockDec", "cl_stock" )
	self.gui:setButtonCallback( "skinStockInc", "cl_stock" )

	self.gui:setButtonCallback( "skinTankDec", "cl_tank" )
	self.gui:setButtonCallback( "skinTankInc", "cl_tank" )

	self.gui:setButtonCallback( "skinSightInc", "cl_sight" )
	self.gui:setButtonCallback( "skinSightDec", "cl_sight" )

	self.gui:setButtonCallback( "skinBarrelDec", "cl_barrel" )
	self.gui:setButtonCallback( "skinBarrelInc", "cl_barrel" )
end

function PotatoGatling:sv_save()
	self.storage:save( self.data )
end

function PotatoGatling:cl_setRenderableTable()
	self.data.renderables = {
		skins.base[self.data.baseIndex],
		skins.stock[self.data.stockIndex],
		skins.tank[self.data.tankIndex],
		skins.sight[self.data.sightIndex],
		skins.barrel[self.data.barrelIndex]
	}

	self:cl_assembleGun()
end

function PotatoGatling:cl_assembleGun()
	local currentRenderablesTp = {}
	local currentRenderablesFp = {}

	for k,v in pairs( renderablesTp ) do currentRenderablesTp[#currentRenderablesTp+1] = v end
	for k,v in pairs( renderablesFp ) do currentRenderablesFp[#currentRenderablesFp+1] = v end
	for k,v in pairs( self.data.renderables ) do currentRenderablesTp[#currentRenderablesTp+1] = v end
	for k,v in pairs( self.data.renderables ) do currentRenderablesFp[#currentRenderablesFp+1] = v end
...
function PotatoGatling:cl_base( button )
	if button == "skinBaseInc" then
		self.data.baseIndex = self.data.baseIndex < #skins.base and self.data.baseIndex + 1 or #skins.base
	else
		self.data.baseIndex = self.data.baseIndex > 1 and self.data.baseIndex - 1 or 1
	end

	self:cl_setRenderableTable()
	self.network:sendToServer("sv_save")
end

function PotatoGatling:cl_stock( button )
	if button == "skinStockInc" then
		self.data.stockIndex = self.data.stockIndex < #skins.stock and self.data.stockIndex + 1 or #skins.stock
	else
		self.data.stockIndex = self.data.stockIndex > 1 and self.data.stockIndex - 1 or 1
	end
...
function PotatoGatling:cl_tank( button )
	if button == "skinTankInc" then
		self.data.tankIndex = self.data.tankIndex < #skins.tank and self.data.tankIndex + 1 or #skins.tank
	else
		self.data.tankIndex = self.data.tankIndex > 1 and self.data.tankIndex - 1 or 1
	end
...
function PotatoGatling:cl_sight( button )
	if button == "skinSightInc" then
		self.data.sightIndex = self.data.sightIndex < #skins.sight and self.data.sightIndex + 1 or #skins.sight
	else
		self.data.sightIndex = self.data.sightIndex > 1 and self.data.sightIndex - 1 or 1
	end
...
function PotatoGatling:cl_barrel( button )
	if button == "skinBarrelInc" then
		self.data.barrelIndex = self.data.barrelIndex < #skins.barrel and self.data.barrelIndex + 1 or #skins.barrel
	else
		self.data.barrelIndex = self.data.barrelIndex > 1 and self.data.barrelIndex - 1 or 1
	end
...
function PotatoGatling.client_onUpdate( self, dt )
	if self.gui:isActive() then
		self.gui:setText( "skinBase", skinNames.base[self.data.baseIndex] )
		self.gui:setText( "skinStock", skinNames.stock[self.data.stockIndex] )
		self.gui:setText( "skinTank", skinNames.tank[self.data.tankIndex] )
		self.gui:setText( "skinSight", skinNames.sight[self.data.sightIndex] )
		self.gui:setText( "skinBarrel", skinNames.barrel[self.data.barrelIndex] )
	end
