...
function ProjectileBtn:server_onFixedUpdate( dt )
	...
	if self.interactable.active and self.count < self.data.outputDuration and self.count > self.data.outputDuration/2 then
		self.poseWeightCount = 0
	elseif self.interactable.active and self.count < self.data.outputDuration and self.count < self.data.outputDuration/2 then
...
function ProjectileBtn:client_onFixedUpdate( dt )
	...
	--print("Pose: ", self.poseWeightCount)
	--print("UV:", self.interactable:getUvFrameIndex())
end
...
function ProjectileBtn:sv_toggle( toggle )
	if toggle then
		...
		self.network:sendToClients( "cl_toggle", true )
		...
		self.network:sendToClients( "cl_toggle", false )
