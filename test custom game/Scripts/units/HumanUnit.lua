...
function HumanUnit.server_onCreate( self )
	...
	print( "-- HumanUnit created --" )
end

function HumanUnit.server_onRefresh( self )
	print( "-- HumanUnit refreshed --" )
end

function HumanUnit.server_onDestroy( self )
	print( "-- HumanUnit terminated --" )
end

function HumanUnit.server_onFixedUpdate( self, dt )
...
function HumanUnit.updateAim( self, dt )
...
function HumanUnit.server_onCharacterChangedColor( self, color )
...
function HumanUnit.server_onUnitUpdate( self, dt )
...
function HumanUnit.sv_e_worldEvent( self, params )
...
function HumanUnit.server_onProjectile( self, hitPos, hitTime, hitVelocity, _, attacker, damage, userData, hitNormal, projectileUuid )
...
function HumanUnit.server_onMelee( self, hitPos, attacker, damage, power, hitDirection )
	if not sm.exists( self.unit ) or not sm.exists( attacker ) then
		return
	end
	local teamOpponent = false
...
function HumanUnit.server_onExplosion( self, center, destructionLevel )
	if not sm.exists( self.unit ) then
		return
	end
...
function HumanUnit.server_onCollision( self, other, collisionPosition, selfPointVelocity, otherPointVelocity, collisionNormal )
	if not sm.exists( self.unit ) then
		return
	end
	...
	if damage > 0 then
		print("'HumanUnit' took", damage, "collision damage")
...
function HumanUnit.server_onCollisionCrush( self )
	if not sm.exists( self.unit ) then
		return
	end
	onCrush( self )
...
function HumanUnit.sv_updateCharacterTarget( self )
	if self.unit.character then
		sm.event.sendToCharacter( self.unit.character, "sv_n_updateTarget", { target = self.target } )
	end
end

function HumanUnit.sv_takeDamage( self, damage, impact, headHit )
	if self.saved.stats.hp > 0 then
		...
		print( "'HumanUnit' received:", damage, "damage.", self.saved.stats.hp, "/", self.saved.stats.maxhp, "HP" )
...
function HumanUnit.sv_spawnParts( self, impact )
	local character = self.unit:getCharacter()
...
function HumanUnit.nearHead( self, hitPos )
	return hitPos.z > self.unit:getCharacter().worldPosition.z + 0.25
end

function HumanUnit.sv_e_receiveTarget( self, params )
