...
---@class GrenadeLauncher : ToolClass
...
local grenadeUUID = sm.uuid.new("27231323-22b2-446b-b27f-7942c47cce46")
...
function GrenadeLauncher.loadAnimations( self )
...
function GrenadeLauncher.client_onUpdate( self, dt )
...
function GrenadeLauncher.client_onEquip( self, animate )
	if animate then
	...
	if self.tool:isLocal() then
		-- Sets GrenadeLauncher renderable, change this to change the mesh
		self.tool:setFpRenderables( currentRenderablesFp )
		swapFpAnimation( self.fpAnimations, "unequip", "equip", 0.2 )
	end
end

function GrenadeLauncher.client_onUnequip( self, animate )
...
function GrenadeLauncher.sv_n_onShoot( self, args )
	local owner = self.tool:getOwner()
	local grenade = sm.shape.createPart( grenadeUUID, args.pos, args.rot, true, true )
	grenade.interactable:setPublicData( { owner = owner.character } )
	sm.physics.applyImpulse(grenade, args.dir, true)
	sm.event.sendToPlayer(owner, "sv_spendAmmo", { type = "grenades", amount = 1 })

	self.network:sendToClients( "cl_n_onShoot" )
end

function GrenadeLauncher.cl_n_onShoot( self )
	if not self.tool:isLocal() and self.tool:isEquipped() then
		self:onShoot()
	end
end

function GrenadeLauncher.onShoot( self )
...
function GrenadeLauncher.calculateFirePosition( self )
...
function GrenadeLauncher.calculateTpMuzzlePos( self )
	local crouching = self.tool:isCrouching()
	local dir = sm.localPlayer.getDirection()
	local pitch = math.asin( dir.z )
	local right = sm.localPlayer.getRight()
...
function GrenadeLauncher.calculateFpMuzzlePos( self )
...
function GrenadeLauncher.cl_onPrimaryUse( self, state )
	local owner = self.tool:getOwner()
	...
	if self.fireCooldownTimer <= 0.0 then
		if owner:getClientPublicData().stats.grenades > -1000 then
			local firstPerson = self.tool:isInFirstPersonView()
			...
			local hit, result = sm.localPlayer.getRaycast(2)
			self.network:sendToServer( "sv_n_onShoot",
				{
					player = owner,
					pos = not hit and sm.camera.getPosition() - sm.vec3.one() / 8 + dir * 2 or result.pointWorld - dir,
...
function GrenadeLauncher.client_onEquippedUpdate( self, primaryState, secondaryState )
	if isAnyOf(primaryState, { 1,2 }) then
