...
function PotatoShotgun.sv_n_onShoot( self, dir )
	sm.event.sendToPlayer(self.tool:getOwner(), "sv_spendAmmo", { type = "shells", amount = 1 })
...
function PotatoShotgun.cl_onPrimaryUse( self, state )
	...
	if self.fireCooldownTimer <= 0.0 then
		...
		if owner:getClientPublicData().stats.shells > 0 then
