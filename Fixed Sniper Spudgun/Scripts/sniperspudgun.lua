...
local Damage = 120
local hintDisplayDuration = 5 * 40

sniperspudgun = class()

local renderables = {
	"$CONTENT_DATA/Tools/Base_sniper/char_spudgun_base_basic.rend",
	"$CONTENT_DATA/Tools/Barrel_sniper/char_spudgun_barrel_basic.rend",
	"$CONTENT_DATA/Tools/Sight_sniper/char_spudgun_sight_basic.rend",
	"$CONTENT_DATA/Tools/Stock_sniper/char_spudgun_stock_broom.rend",
	"$CONTENT_DATA/Tools/Tank_sniper/char_spudgun_tank_basic.rend"
}
...
function sniperspudgun.client_onCreate( self )
	self.shootEffect = sm.effect.createEffect( "TapeBote - Ukrainian" )
	self.shootEffectFP = sm.effect.createEffect( "TapeBote - Ukraine" )

	if not self.tool:isLocal() then return end

	self.hintBeginTick = nil
	self.zoomlevel = 20
end



function sniperspudgun.client_onRefresh( self )
	self:loadAnimations()
end

function sniperspudgun.loadAnimations( self )
...
function sniperspudgun.client_onUpdate( self, dt )
	if self.hintBeginTick ~= nil and sm.game.getCurrentTick() < self.hintBeginTick + hintDisplayDuration then
		sm.gui.setInteractionText( "", sm.gui.getKeyBinding( "NextCreateRotation", true ), "- zoom in" )
		sm.gui.setInteractionText( "", sm.gui.getKeyBinding( "Reload", true ), "- zoom out" )
	end
	...
	self.tool:updateCamera( 2.8, 16.0, sm.vec3.new( 0.65, 0.0, 0.05 ), self.aimWeight )
	self.tool:updateFpCamera( self.zoomlevel, sm.vec3.new( 0.0, 0.0, 0.0 ), self.aimWeight, bobbing )
end

function sniperspudgun.client_onEquip( self )
	...
	for k,v in pairs( renderablesFp ) do rendFp[#rendFp+1] = v end
	for k,v in pairs( renderablesTp ) do rendTp[#rendTp+1] = v end
	for k,v in pairs( renderables ) do rendFp[#rendFp+1] = v end
	for k,v in pairs( renderables ) do rendTp[#rendTp+1] = v end
	self.tool:setTpRenderables( rendTp )
	...
	if self.tool:isLocal() then
		-- Sets sniperspudgun renderable, change this to change the mesh
		self.tool:setFpRenderables( rendFp )
		swapFpAnimation( self.fpAnimations, "unequip", "equip", 0.2 )
	end
end

function sniperspudgun.client_onUnequip( self )
...
function sniperspudgun.sv_n_onAim( self, aiming )
	self.network:sendToClients( "cl_n_onAim", aiming )
end

function sniperspudgun.cl_n_onAim( self, aiming )
	if not self.tool:isLocal() then
		self:onAim( aiming )
	end
end

function sniperspudgun.client_onReload( self )
    if self.zoomlevel < 30 and self.tool:isInFirstPersonView() then -- max zoom level
		sm.gui.startFadeToBlack( 0.25, 0.35 )
...
function sniperspudgun.client_onToggle( self )
    if self.zoomlevel > 10 and self.tool:isInFirstPersonView() then -- min zoom level
		sm.gui.startFadeToBlack( 0.25, 0.35 )
        self.zoomlevel = self.zoomlevel - 2
    end
...
function sniperspudgun.onAim( self, aiming )
	self.aiming = aiming
...
function sniperspudgun.sv_n_onShoot( self, dir )
	self.network:sendToClients( "cl_n_onShoot", dir )
end

function sniperspudgun.cl_n_onShoot( self, dir )
	if not self.tool:isLocal() then
		self:onShoot( dir )
	end
end

function sniperspudgun.onShoot( self, dir )
...
function sniperspudgun.calculateFirePosition( self )
...
function sniperspudgun.calculateTpMuzzlePos( self )
	local crouching = self.tool:isCrouching()
	local dir = sm.localPlayer.getDirection()
	local pitch = math.asin( dir.z )
	local right = sm.localPlayer.getRight()
...
function sniperspudgun.calculateFpMuzzlePos( self )
...
function sniperspudgun.cl_onPrimaryUse( self, state )
...
function sniperspudgun.cl_onSecondaryUse( self, state )
	if state == sm.tool.interactState.start and not self.aiming then
		self.zoomlevel = 16

		local json = sm.json.open("$CONTENT_DATA/Scripts/userData.json")
		if json.displayHint then
			sm.json.save( { displayHint = false }, "$CONTENT_DATA/Scripts/userData.json" )
			self.hintBeginTick = sm.game.getCurrentTick()
		end
...
function sniperspudgun.client_onEquippedUpdate( self, primaryState, secondaryState )
	if self.aiming then
		sm.gui.displayAlertText( "Zoom: #df7f00"..tostring(self.zoomlevel / 16).."x", 1 )
	end
