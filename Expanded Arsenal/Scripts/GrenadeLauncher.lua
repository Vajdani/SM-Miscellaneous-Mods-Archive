...
local grenades = {
	sm.uuid.new("20c3398b-7b00-4861-ac98-2a55495535b4"),
	sm.uuid.new("c0dcaf49-cc53-404f-931f-255e1fa8f6b5")
}
...
function Launcher.client_onCreate( self )
	...
	self.cl = {}
	self.cl.currentGrenade = 1
end

function Launcher:client_onToggle()
	self.cl.currentGrenade = self.cl.currentGrenade == #grenades and 1 or self.cl.currentGrenade + 1
	sm.gui.displayAlertText("Current grenade: #df7f00"..sm.shape.getShapeTitle( grenades[self.cl.currentGrenade] ), 2.5)

	sm.audio.play("PaintTool - ColorPick")
	return true
end

function Launcher:sv_throwGrenade( args )
	local grenade = sm.shape.createPart( args.uuid, args.pos, args.rot, args.dynamic, args.forceSpawn )
	sm.physics.applyImpulse(grenade, args.dir, true)

	sm.container.beginTransaction()
	sm.container.spend( args.player:getInventory(), args.uuid, 1 )
	sm.container.endTransaction()
end

function Launcher.client_onRefresh( self )
	self:loadAnimations()
end

function Launcher.loadAnimations( self )
...
function Launcher.client_onEquip( self, animate )
	...
	if self.tool:isLocal() then
		-- Sets Launcher renderable, change this to change the mesh
		self.tool:setFpRenderables( currentRenderablesFp )
		swapFpAnimation( self.fpAnimations, "unequip", "equip", 0.2 )
	end
end

function Launcher.client_onUnequip( self, animate )
...
function Launcher.sv_n_onAim( self, aiming )
	self.network:sendToClients( "cl_n_onAim", aiming )
end

function Launcher.cl_n_onAim( self, aiming )
	if not self.tool:isLocal() and self.tool:isEquipped() then
		self:onAim( aiming )
	end
end

function Launcher.onAim( self, aiming )
	self.aiming = aiming
...
function Launcher.sv_n_onShoot( self, dir )
	self.network:sendToClients( "cl_n_onShoot", dir )
end

function Launcher.cl_n_onShoot( self, dir )
	if not self.tool:isLocal() and self.tool:isEquipped() then
		self:onShoot( dir )
	end
end

function Launcher.onShoot( self, dir )
...
function Launcher.calculateFirePosition( self )
...
function Launcher.calculateTpMuzzlePos( self )
	local crouching = self.tool:isCrouching()
	local dir = sm.localPlayer.getDirection()
	local pitch = math.asin( dir.z )
	local right = sm.localPlayer.getRight()
...
function Launcher.calculateFpMuzzlePos( self )
...
function Launcher.cl_onPrimaryUse( self, state )
	if self.tool:getOwner().character == nil then
		return
	end

	if self.fireCooldownTimer <= 0.0 and state == sm.tool.interactState.start then
		local grenade = grenades[self.cl.currentGrenade]

		if not sm.game.getEnableAmmoConsumption() or sm.container.canSpend( sm.localPlayer.getInventory(), grenade, 1 ) then
			local firstPerson = self.tool:isInFirstPersonView()
			...
			if owner then
				...
				self.network:sendToServer("sv_throwGrenade",
					{
						player = sm.localPlayer.getPlayer(),
						uuid = grenade,
						pos = sm.camera.getPosition() - sm.vec3.one() / 4 + dir,
...
function Launcher.cl_onSecondaryUse( self, state )
...
function Launcher.client_onEquippedUpdate( self, primaryState, secondaryState )
