...
local renderables =   {
	"$SURVIVAL_DATA/Character/Char_Glowstick/char_glowstick.rend",
	"$SURVIVAL_DATA/Character/Char_Glowstick/char_glowstick.rend", --grenade
	"$SURVIVAL_DATA/Character/Char_Glowstick/char_glowstick.rend" --molotov
}
...
function Glowstick.cl_init( self )
	...
	--EG
	self.cl = {}
	self.cl.renderable = { renderables[self.data.renderableIndex] }

	if self.data.item == "glowstick" then
...
function Glowstick.client_onEquip( self )
	...
	for k,v in pairs( self.cl.renderable ) do currentRenderablesTp[#currentRenderablesTp+1] = v end
	for k,v in pairs( self.cl.renderable ) do currentRenderablesFp[#currentRenderablesFp+1] = v end
					...
					self.network:sendToServer("sv_throwGrenade",
						{
							...
							uuid = sm.uuid.new(self.data.item),
...
--EG
function Glowstick:sv_throwGrenade( args )
